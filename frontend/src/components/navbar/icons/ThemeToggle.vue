...
// 更新 DOM 的 class 和本地存储
const updateDOM = () => {
  const docElement = document.documentElement;
  if (isDarkMode.value) {
    // 兼容可能存在的 tailwind 'dark:' 类
    docElement.classList.add('dark');
    // 设置 DaisyUI 的暗黑主题，全局生效！
...
onMounted(() => {
  try {
    ...
    // 如果本地有记录，则使用记录；如果没有记录，可根据系统偏好自动设置
    if (savedTheme === 'dark') {
...
.theme-toggle {
  /* 重要修改：这里将原本极其巨大的 120px 改为 10px。*/
  /* 如果在 Navbar 里觉得太小或太大，只需要微调这里的数字即可（比如改到 12px 或 8px）*/
  --base-scale: 10px;
